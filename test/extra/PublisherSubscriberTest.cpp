...
v1::UUri makeUUri(std::string_view authority, uint16_t ue_id, uint16_t ue_instance, uint8_t version, uint16_t resource) {
	v1::UUri uuri;
...
v1::UUri makeUUri(std::string_view serialized) {
	return datamodel::serializer::uri::AsString::deserialize(static_cast<std::string>(serialized));
}

std::shared_ptr<transport::UTransport> getTransport(const v1::UUri& uuri = makeUUri(ENTITY_URI_STR)) {
	return std::make_shared<transport::ZenohUTransport>(uuri, ZENOH_CONFIG_FILE);
}
...
TEST_F(PublisherSubscriberTest, SinglePubSingleSub) {
	...
	communication::Publisher pub(transport, makeUUri(TOPIC_URI_STR), v1::UPayloadFormat::UPAYLOAD_FORMAT_TEXT);
	...
	auto maybe_sub = communication::Subscriber::subscribe(transport, makeUUri(TOPIC_URI_STR), std::move(on_rx));
	...
	for (auto remaining = num_publish_messages; remaining > 0; --remaining) {
		...
		auto result = pub.publish({std::move(message).str(), v1::UPayloadFormat::UPAYLOAD_FORMAT_TEXT});
