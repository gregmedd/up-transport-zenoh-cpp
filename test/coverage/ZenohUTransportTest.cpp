...
TEST_F(TestZenohUTransport, ConstructDestroy) {
	...
	auto transport = std::make_shared<transport::ZenohUTransport>(create_uuri(ENTITY_URI_STR), ZENOH_CONFIG_FILE);
}

struct ExposeKeyString : public transport::ZenohUTransport {
	template<typename... Args>
	static auto toZenohKeyString(Args&&... args) {
		return transport::ZenohUTransport::toZenohKeyString(std::forward<Args>(args)...);
	}
};

TEST_F(TestZenohUTransport, toZenohKeyString) {
	EXPECT_TRUE((std::is_base_of_v<transport::ZenohUTransport, ExposeKeyString>));
